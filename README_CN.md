...
一个 Skill：先了解你的项目，按需问几个问题（你也可以选择直接用默认值跳过），自动挑选合适的 README 类型，输出产品级双语 README + 配套图片任务清单。
...
这个 Skill 把这套"设计产品页"的手艺固化下来。它先扫描项目自动画像，按需问你最多 5 个问题（你也可以直接全选默认），自动挑选项目类型，跑反模式自检，输出中英双版。
...
- **17 项反模式自检** —— 唯一真理源在 `references/principles.md`，生成草稿前自动跑。能自动改的直接修，主观判断的升级到人工评审。
- **访谈强度可调** —— 默认按扫描结果按需提问最多 5 个，可一键选"全用默认"直接跳过；也可切换到深度模式补充更多上下文。
...
| 维度 | 选项 | 默认 |
|---|---|---|
| **流程** | `--quick`（按需问 1-5 个） / `--full`（深度访谈，最多 7 个） / `--rewrite`（保留金句、改弱段、补缺漏） / `--patch`（只补缺失，不访谈） | `--quick` |
| **输出语言** | `--bilingual` / `--en-only` / `--zh-only` | `--bilingual` |
| **图片任务** | `--with-images` / `--no-images` | `--with-images` |

提问数量是按项目情况动态决定的，不是固定。每次访谈开头 Skill 都会让你选："要不要答几个问题？"——你可以选答、选只答关键的、或直接全用默认值跳过。

```bash
/jvs-readme-designer                       # 默认：双语 + 图片清单 + 按需访谈
/jvs-readme-designer --full                # 深度访谈
...
```bash
# 克隆到你自己习惯的代码目录
git clone https://github.com/<owner>/jvs-readme-designer.git <你的代码目录>/jvs-readme-designer

# 符号链接
ln -s <你的代码目录>/jvs-readme-designer/skills/jvs-readme-designer \
      ~/.claude/skills/jvs-readme-designer
```

Cursor / Codex 等其他 AI 编辑器：参考各自文档把 `skills/jvs-readme-designer` 注册为 skill 即可。
...
| **⛔ INTERVIEW** | 按需提问，最多 5 个；开头让你选"答几个 / 只答关键的 / 全用默认"。已知答案不再问。 |
...
└── skills/
    └── jvs-readme-designer/           # 实际 skill 内容
        ├── SKILL.md                   # 主入口，5 阶段流程
        ├── EXTEND.md                  # 用户偏好持久化模板
        ├── _INDEX.md
        └── references/
...
- **v2.1**（当前）—— 生产可用。通过了 4 路审稿 + 验证轮。
- **v3**（计划中）—— 量化评估：在真实开源项目样本集上跑 Skill，按 17 项 checklist 自动评分。
