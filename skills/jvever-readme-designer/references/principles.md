...
**应用要点**：
- **段落必要性 sweep（最关键）**：DRAFT 完成后，对每个 `##` 段问——"如果删掉这整段，目标读者损失什么具体能帮他做决策的信息？"答不出 / 答出"了解作者 / 知道项目结构 / 看到设计哲学" → **删整段**，不保留。**README 是 landing page，不是 documentation**。常见可疑段（默认应当审视并多半删除）：
