...
> 下方 6 条信念是方向；判定方法见 [`references/principles.md`](references/principles.md) 顶部 **2 条元原则**——
> - **元原则 A**：必要性证明 > 完整性焦虑（每条输出/规则/段都要回答"删了读者损失什么"）
> - **元原则 B**：判断优于查表（清单都是启发，不是穷举的匹配规则）
>
> 任何 specific 规则与元原则冲突时，**元原则胜出**。新场景下不要查规则清单——回到元原则判断。
...
下列字段**只要触发条件命中、且无法从仓库直接读出，就必须问**，不允许走默认值/⚠️ 标记。这是对 1.3 软性自评的兜底——它们的共同特点是"会渲染在 README 中且具有法律 / 对外属性，错了不只是质量打折"。

| 字段 | 触发条件 | 默认行为不可接受的原因 |
|---|---|---|
| **License** | 仓库根无 `LICENSE` / `LICENSE.md` / `LICENSE.txt`，且 README 中需要展示 license badge / 段落 | "TBD" 在 public 仓库默认 = all rights reserved，与多数 OSS 作者意图（MIT / Apache-2.0 / GPL / CC BY-SA）相反；License 一旦写入 LICENSE 文件即对外生效 |
| **目标用户口径确认** | manifest description 与 SCAN 推断的 archetype 矛盾，或现有 README 已有冲突表述 | 错了会让 hero 段定位失准 |
| **公开发布意图** | 仓库当前 `private` 但用户语境表明"准备发布 / 上线" | 影响是否启用 trust signal 段、社区入口、Star History 触发条件等 |

注：**License 不是"先生成 TBD 后续再补"的字段**——它必须在 DRAFT 之前确定。如果用户跳过追问，REVIEW 报告里要把 license 列为 🔴 **未决断的法律字段**（比 ⚠️ 更高一档），而不是当作普通的"走默认值"。
...
| 类别 | 例子 | 判断标准 |
|---|---|---|
| **客观未知** | License 选哪个（见上方硬性触发器）？是否真有可贴的大客户名单 / testimonial？monorepo 为根还是子包写？ | 不能从代码推断的事实 |
...
#### 2.8 DRAFT 自检（**先元原则反向审视**，再机械、再主观）

DRAFT 完成后必须跑三层自检，**全部通过**才能进入 REVIEW。

**第零层：元原则 A 反向审视（强制必跑、按粒度）**

DRAFT 完成的第一件事，**不是读完检查规则**——是回到 [`principles.md`](references/principles.md) 顶部的**元原则 A**，按 5 个粒度逐项**反向证明**。每跑完一个粒度，把违反的项**直接修掉或删掉**，再跑下一个粒度。

| 粒度 | 反向证明问 | 高频违反 |
|---|---|---|
| **段** | 把每个 `##` 段独立读一遍，问"删了它读者损失什么具体能帮他做决策的信息？答不出 → 删整段" | 项目结构 / 怎么做出来 / 维护提醒 / 何时不用 / "它解决了什么"教育段 / 讲 Skill 工作流程的元描述段 |
| **段间重复** | "Hero 区出现的内容（如 install 命令）是否在下方 Quickstart 段又出现？同一信息出现两次 → 留一处删另一处" | Hero 完整 install 块 + Quickstart 段又一个完整 install 块 |
| **bullet** | "每条单独读，删了它读者会反向惊讶'居然没有？'吗？会的话是基线特征/必备款，砍掉" | 中英双语在 i18n 工具品类 / 平台 metadata / 输出形式不是钩子 |
| **句 / 长句** | "这句话告诉读者得到什么，还是告诉他作者怎么实现 / 怎么想？是后者就重写" | 讲方法论（"Hero 当销售页写不当目录写"）/ 讲实现机制（"两层自检""manifest 解析"）|
| **词 / 术语 / 内部数字** | "项目内部术语 / 内部数字 / 内部分类编号目标读者认得吗？换成他熟悉的词更易 grok 吗？" | `archetype` / `tagline 套路` / `landing page` / "5 种""6 种""两层"等暴露内部 spec 的数字 |
| **命令** | "对最不熟该平台的目标读者也跑得起来吗？还是要他先懂别的（软链 / 系统变量 / 包管理器细节）？" | `ln -s "$(pwd)/..."` / 复杂 sed/awk pipeline / 假定目标读者熟悉的工具 |

**走完这层之后**，再跑下面两层。**不允许跳过这层直接跑机械检测**——元原则的强制 enforce 比 specific 规则更重要。
...
用户指出某个词不好时，**修这一处的同时一并扫同义词族**——但**不主动改其他段落的措辞**。区分清楚：

- ✅ 用户说"'全队'听起来怪" → 修这处的同时全文搜同义"全员 / 团队"，**只在指出的同类语境**下一并改（不延伸到其他无关段落）
- ✅ 用户说"'芯片'不该直译" → 同处改的同时扫"快捷选项 / 标签 / 分类"等同源直译位置，**统一术语**
- ❌ 用户说"首屏段太营销了" → 不要扩散去改特性段 / 为什么段（除非用户说"全篇都太营销"）
- ❌ 用户没指出问题，模型不要主动扫"可能其他地方也有问题"
